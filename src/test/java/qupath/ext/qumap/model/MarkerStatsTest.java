...
class MarkerStatsTest {
    ...
    void computeWithKnownDistribution() {
        ...
        assertEquals(Math.sqrt(2.5), stats.getStd("CD45"), 0.001);
    ...
    void zScoreConversion() {
        ...
        assertEquals(2.0 / Math.sqrt(2.5), stats.toZScore("CD45", 5.0), 0.001);
    }
    ...
    @Test
    void sampleStdDevUsed() {
        // Values {1, 2, 3, 4, 5}: mean=3, sample variance = 10/4 = 2.5, sample std = sqrt(2.5)
        CellIndex index = buildIndex(List.of("CD45"), new double[][]{{1, 2, 3, 4, 5}});
        MarkerStats stats = MarkerStats.compute(index);
        assertEquals(Math.sqrt(2.5), stats.getStd("CD45"), 0.001);
    }
    ...
    @Test
    void singleCellZScoreReturnsZero() {
        CellIndex index = buildIndex(List.of("CD45"), new double[][]{{7.0}});
        MarkerStats stats = MarkerStats.compute(index);
        // Single cell: std = 0, z-score should return 0 (no variance)
        assertEquals(0.0, stats.toZScore("CD45", 7.0), 0.001);
    }

    @Test
    void getMarkerNamesReturnsUnmodifiableSet() {
        CellIndex index = buildIndex(List.of("CD45"), new double[][]{{1, 2, 3}});
        MarkerStats stats = MarkerStats.compute(index);
        Set<String> names = stats.getMarkerNames();
        assertThrows(UnsupportedOperationException.class, () -> names.add("HACKED"));
    }

    @Test
    void twoCellsSampleStdDev() {
        // Values {0, 2}: mean=1, sample variance = ((0-1)^2 + (2-1)^2) / (2-1) = 2, std = sqrt(2)
        CellIndex index = buildIndex(List.of("CD45"), new double[][]{{0, 2}});
        MarkerStats stats = MarkerStats.compute(index);
